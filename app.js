...
(function attachDelegatedPaperSubmitFallback() {
    ...
    document.addEventListener('submit', async function delegatedPaperSubmitHandler(e) {
        ...
        const form = e.target;
        if (!form || form.id !== 'paperUploadForm') return;

        e.preventDefault();
        e.stopImmediatePropagation();
        console.log("Delegated handler: caught submit for #paperUploadForm");

        // Defensive reads
        const get = id => document.getElementById(id);
        const titleEl = get('paperTitle');
        const authorsEl = get('paperAuthors');
        const abstractEl = get('paperAbstract');
        const categoryEl = get('paperCategory');
        const tagsEl = get('paperTags');
        const urlEl = get('paperFileUrl');
        ...
        console.log("Delegated submit values:", { title, authors, category, abstract, tags, url });
        ...
        try {
            ...
            // Do the same addDoc call as your normal handler
    ...
    (function attachSubmitButtonBackup() {
        const tryAttach = () => {
            const form = document.getElementById('paperUploadForm');
            ...
            btn.addEventListener('click', (ev) => {
                ...
                const f = document.getElementById('paperUploadForm');
                if (!f) return;
                if (!f.checkValidity()) {
